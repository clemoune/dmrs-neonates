function [E, J] = SynthMeasTortZeppelinCylinderBallB0(params,protocol,params_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating a TortZeppelinCylinderBallB0 model.
% 
% [E,J]=SynthMeasTortZeppelinCylinderBallB0(params, protocol,params_deriv)
% 
...
if nargout == 1           
        E = b0*((1-ficvf)*(1-fiso)*Zeppelin(params_zep,protocol)+ficvf*(1-fiso)*Cylinder(params_cyl,protocol)+ fiso*Ball(params_ball,protocol));
else 
    if nargin <3
    ...
    if params_deriv(1) % ficvf volume fraction, including turtuosity term
        it = it+1; 
        J(:,it) = b0*((1-fiso)*E2 - (1-fiso)*E1 + (1-fiso)*(1-ficvf)*J1(:,2)*(-params_zep(1))); 
    end 
    if params_deriv(2) % di, including turtuosity term
        it = it+1; 
        J(:,it) = b0*((1-ficvf)*(1-fiso)*J1(:,1) + ficvf*(1-fiso)*J2(:,1) + (1-ficvf).^2*(1-fiso)*J1(:,2)) ; 
    end 
    if params_deriv(3) % radius
        it = it+1;
        J(:,it) = b0*(ficvf*(1-fiso)*J2(:,2)); 
    end 
    if params_deriv(4) % fiso
        it = it+1; 
        J(:,it) = b0*(-(1-ficvf)*E1 - ficvf* E2 + E3); 
    end 
    ...
    if params_deriv(6) % theta
        it = it+1; 
        J(:,it) = b0*((1-ficvf)*(1-fiso)*J1(:,3) + ficvf*(1-fiso)*J2(:,3)); 
    end 
    if params_deriv(7) % phi
        it = it+1; 
        J(:,it) = b0*((1-ficvf)*(1-fiso)*J1(:,4) + ficvf*(1-fiso)*J2(:,4)); 
    end  
