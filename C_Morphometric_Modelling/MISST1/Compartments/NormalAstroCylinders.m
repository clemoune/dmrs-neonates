function [E,J]=NormalAstroCylinders(x, protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating the NormalAstroCylinders compartment.
% 
% [E,J]=NormalAstroCylinders(x, protocol,x_deriv)
% 
% Description: Returns the diffusion measurement and the Jacobian with 
% respect to model parameters for a substrate consisting of isotropically 
% oriented cylinders with a normal distribution of radii and a diffusion 
% protocol specified in the input
% Substrate: Isotropically distributed impermeable cylinders with a normal
...
% x - size 3 vector of model parameters in SI units for NormalAstroCylinders:
    ...
    if(nargout>1)
        ...
        if nargin < 3          
            for i = 1:length(x); % compute the derivatives for all model parameters
                ...
                Epert = NormalAstroCylinders(xpert, protocol);
...
else % signal computed using MM
    fun = str2func(['NormalCylinders_' approx '_' protocol.pulseseq]);
    Ndir = 20;    
    fibre_dirs = ReadCaminoElecPS(sprintf('PointSets/Elec%03i.txt',Ndir))';  
    [az incl] = cart2sph(fibre_dirs(1,:),fibre_dirs(2,:),fibre_dirs(3,:)); % azimuth and inclinations
