function [E J]=NormalCylinders_MM_dPGSE(x,protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating the NormalCylinders compartment.
% 
% [E,J]=NormalCylinders_MM_dPGSE(x, protocol,x_deriv)
% 
% Description: Returns the diffusion measurement and the Jacobian with 
% respect to model parameters for a substrate consisting of parallel
% cylinders with a normal distribution of radii and a diffusion 
% protocol specified in the input
% Substrate: Parallel, impermeable cylinders with a normal distribution of radii
...
% x - size 5 vector of model parameters in SI units for NormalCylinders:
%       x(1) - free diffusivity of the material inside the cylinders.
%       x(2) - mean radius of the distribution
%       x(3) - variance of the distribution
...
fibredir = GetFibreOrientation('NormalCylinders',x);
...
for indj = 1:length(protocol.A_cyl)
    ...
    elseif protocol.diff==3
        Amat_cyl=protocol.Apertvar_cyl{indj};
        Smat_cyl=protocol.Spertvar_cyl{indj};
        Rmat_cyl=protocol.Rpertvar_cyl{indj};  
        Rweight = repmat(protocol.Rweight_pertvar,[M,1]);
...
if(nargout>1)
    ...
    if nargin < 3 
        ...
        Epert = NormalCylinders_MM_dPGSE(xpert,protocol);
        ...
        for i = 1:length(x)    
            if x_deriv(i) ~= 0                
                ...
                xpert(i) = xpert(i)*(1+dx);    
                Epert = NormalCylinders_MM_dPGSE(xpert,protocol,x_deriv);
                dEtdx = (Epert - E)/(xpert(i)*dx);
                J(:,i) = dEtdx;
            end
        end
