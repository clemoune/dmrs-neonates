function [E J]=FiniteCylinder_MM_GEN(x,protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating a finite cylinder compartment.
% 
% [E,J]=FiniteCylinder_MM_GEN(x, protocol,x_deriv)
% 
...
if(nargout>1)
    ...
    if nargin < 3 
        ...
        Epert = FiniteCylinder_MM_GEN(xpert,protocol);
        ...
        for i = 1:length(x)
            if x_deriv(i) ~= 0                  
                ...
                xpert(i) = xpert(i)*(1+dx);    
                Epert = FiniteCylinder_MM_GEN(xpert,protocol,x_deriv);
                dEtdx = (Epert - E)/(xpert(i)*dx);
                J(:,i) = dEtdx;
            end
        end
