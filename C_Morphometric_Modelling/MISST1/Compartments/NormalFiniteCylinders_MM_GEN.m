function [E J]=NormalFiniteCylinders_MM_GEN(x,protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating a NormalFiniteCylinder compartment.
% 
% [E,J]=NormalFiniteCylinder_MM_GEN(x, protocol,x_deriv)
% 
...
if isfield(protocol,'smalldel') && isfield(protocol,'delta')
    ...
    for indj = 1:length(protocol.A_cyl)
        ...
        elseif protocol.diff==3
            ...
            Amat_plane=protocol.Apertvar_plane{indj};
            Smat_plane=protocol.Spertvar_plane{indj};
            Rmat_plane=protocol.Rpertvar_plane{indj};     
...
if(nargout>1)
    ...
    if nargin < 3 
        ...
        Epert = NormalFiniteCylinders_MM_GEN(xpert,protocol);
        ...
        for i = 1:length(x)
            if x_deriv(i) ~= 0                  
                ...
                xpert(i) = xpert(i)*(1+dx);    
                Epert = NormalFiniteCylinders_MM_GEN(xpert,protocol,x_deriv);
                dEtdx = (Epert - E)/(xpert(i)*dx);
                J(:,i) = dEtdx;
            end
        end
