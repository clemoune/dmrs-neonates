function [E,J]=Sphere_GPD_FullSTEAM(x,protocol)
...
% Components of gradients along x,y,z
zdir = [0 0 1]';
cosTheta = grad_dirs*zdir;
...
fibdirrep = repmat(zdir',[size(grad_dirs,1) 1]);
...
comp1_dirs(fgpar, :) = repmat([zdir(3) zdir(3) zdir(1)+zdir(2)], [length(fgpar) 1]);
...
% Log of attenuation from gradients along z
logE_Par = -2*GAMMA^2.*(sGG.*(GD_CompPar.^2) + sGGc.*GD_CompPar.*GC_CompPar + ...
                                              sGcGc.*(GC_CompPar.^2) + sGGr.*GD_CompPar.*GR_CompPar + ...
                                              sGrGr.*(GR_CompPar.^2) + sGcGr.*GC_CompPar.*GR_CompPar);
...
ePerp = exp(logE_Par+ logE_Comp1 + logE_Comp2);
...
if(nargout>1)
    ...
    Epert = SynthMeasCylSingleRadGPD_DSE(xpert, grad_dirs, G, TE, delta1, delta2, delta3, t1, t2, t3, zdir, roots);
