function [E J]=GammaCylinders_MM_DODE(x,protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
...
fib_dir = GetFibreOrientation('GammaCylinders',x);
...
if(nargout>1)
    ...
    if nargin < 3 
        ...
        Epert = GammaCylinders_MM_DODE(xpert,protocol);
        ...
        for i = 1:length(x)    
            if x_deriv(i) ~= 0                
                ...
                xpert(i) = xpert(i)*(1+dx);    
                Epert = GammaCylinders_MM_DODE(xpert,protocol,x_deriv);
                dEtdx = (Epert - E)/(xpert(i)*dx);
                J(:,i) = dEtdx;
            end
        end
