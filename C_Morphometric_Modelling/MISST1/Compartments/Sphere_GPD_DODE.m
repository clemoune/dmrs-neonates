function [E,J]=Sphere_GPD_DODE(x, protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
...
exp_tm_smalldel_NT = exp(-amSq.*dRes.*(tmmx_rep+smalldelmx_rep-NTmx_rep./(2.*freqmx_rep)));
...
sumterms = 2.*(G1mx_rep.^2+G2mx_rep.^2).*(-1 + exp_smalldel_NT + NTmx_rep.*(-1+exp_1) + am_dRes.*smalldelmx_rep)./am2_dRes2;

% corresponds to the multiplication of the full oscillations with
% other full oscillations from the same gradient
sumterms = sumterms - 2.*(G1mx_rep.^2+G2mx_rep.^2).* sgn.*(1-exp_1).^2./am2_dRes2./(1+exp_1).^2.*...
    (exp_NT + sgn.*(NTmx_rep-1)+sgn.*NTmx_rep.*exp_1);

% corresponds to the multiplication of the full oscillations with
% other full oscillations from different gradients
sumterms = sumterms - 2.*G1mx_rep.*G2mx_rep.*CosPsimx_rep.*exp_tm_smalldel_NT.*(1-exp_1).^2.*(1-sgn.*exp_NT).*(sgn-exp_NT)./...
    (1+exp_1).^2./am2_dRes2;
...
if(nargout>1)
    ...
    if nargin < 3 
        ...
        for i = 1:2; % compute the derivatives for all model parameters
            ...
            Epert = Sphere_GPD_DODE(xpert, protocol);
