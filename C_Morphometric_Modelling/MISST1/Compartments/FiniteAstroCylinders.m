function [E,J]=FiniteAstroCylinders(x, protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating the FiniteAstroCylinders compartment.
% 
% [E,J]=FiniteAstroCylinders(x, protocol,x_deriv)
% 
...
% Substrate: Isotropically distributed impermeable finite cylinders
% Diffusion pulse sequence: wrapper for various sequences
%
% Parameters:   
% E - diffusion signal
% J - Jacobian of the diffusion signal with respect to model parameters
% x - size 3 vector of model parameters in SI units for FiniteAstroCylinders:
    ...
    if(nargout>1)
        ...
        if nargin < 3          
            for i = 1:3; % compute the derivatives for all model parameters
                ...
                Epert = FiniteAstroCylinders(xpert, protocol);
            ...
            J(:,i) = dEtdx;
             ...
             for i = 1:length(x)        
                if x_deriv(i) ~= 0  
                    ...
                    protocol.diff=i;   
                    xpert(i) = xpert(i)*(1+dx);    
                    Epert = FiniteAstroCylinders(xpert,protocol,x_deriv);
                    dEtdx = (Epert - E)/(xpert(i)*dx);
                    J(:,i) = dEtdx;
                end     
