function [E J]=CuboidSym_MM_SWOGSE_3D(x,protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating a cuboid/ensemble of cuboids with 2 equal sides
% 
% [E,J]=CuboidSym_MM_SWOGSE_3D(x, protocol,x_deriv)
% 
...
elseif protocol.diff==2
    ...
    Amat_y=protocol.Aperta_y{indj};
    Smat_y=protocol.Sperta_y{indj};
    Rmat_y=protocol.Rperta_y{indj};
    Amat_z=protocol.Aperta_z{indj};
    Smat_z=protocol.Sperta_z{indj};
    Rmat_z=protocol.Rperta_z{indj}; 
...
elseif protocol.diff==3
    ...
    Smat_y=protocol.S_y{indj};
    Rmat_y=protocol.R_y{indj};
    Amat_z=protocol.ApertEcc_z{indj};
    Smat_z=protocol.SpertEcc_z{indj};
    Rmat_z=protocol.RpertEcc_z{indj}; 
...
if(nargout>1)
    ...
    if nargin < 3 
        ...
        Epert = CuboidSym_MM_SWOGSE_3D(xpert,protocol);
        ...
        for i = 1:3            
            if x_deriv(i) ~= 0  
                ...
                protocol.diff=i;   
                xpert(i) = xpert(i)*(1+dx);    
                Epert = CuboidSym_MM_SWOGSE_3D(xpert,protocol,x_deriv);
                dEtdx = (Epert - E)/(xpert(i)*dx);
                J(:,i) = dEtdx;
            end
        end
