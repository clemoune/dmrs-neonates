function [E J]=CuboidSym_MM_GEN(x,protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating a cuboid/ensemble of cuboids with 2 equal sides
% 
% [E,J]=CuboidSym_MM_GEN(x, protocol,x_deriv)
% 
...
if(nargout>1)
    ...
    if nargin < 3 
        ...
        Epert = CuboidSym_MM_GEN(xpert,protocol);
        ...
        for i = 1:3            
            if x_deriv(i) ~= 0  
                ...
                protocol.diff=i;   
                xpert(i) = xpert(i)*(1+dx);    
                Epert = CuboidSym_MM_GEN(xpert,protocol,x_deriv);
                dEtdx = (Epert - E)/(xpert(i)*dx);
                J(:,i) = dEtdx;
            end
        end
