function [E,J]=Cylinder_GPD_tPGSE(x, protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating a cylinder compartment.
% 
% [E,J]=Cylinder_MM_tPGSE(x, protocol,x_deriv)
% 
...
bval_par = GAMMA.^2.*(G1mx_par.^2+G2mx_par.^2+G3mx_par.^2).*smalldelmx.^2.*(deltamx-smalldelmx/3);
...
if(nargout>1)
    ...
    if nargin < 3 
        ...
        for i = 1:4; % compute the derivatives for all model parameters
            ...
            Epert = Cylinder_GPD_tPGSE(xpert, protocol);
