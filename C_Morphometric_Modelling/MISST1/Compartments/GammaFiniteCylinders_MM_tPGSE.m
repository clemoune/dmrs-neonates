function [E J]=GammaFiniteCylinders_MM_tPGSE(x,protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating a GammaFiniteCylinders compartment.
% 
% [E,J]=GammaFiniteCylinders_GPD_tPGSE(x, protocol,x_deriv)
% 
           ...
           AmatUD3_plane=AmatU3_plane^G_ind3(m);
             ...
             if tm12(m) >= smalldel1(m) % no overlap between the two PGSEs
                 ...
                 Ecal_plane=Ecal_plane*(Rmat_plane*AmatUD1_plane)^tmp1(m)*Rmat_plane^dstmp1(m)*(Rmat_plane*AmatUD1_plane')^tmp1(m)*... % first PGSE
                     Rmat_plane^dstmp12*(Rmat_plane*AmatUD2_plane')^tmp2(m)*Rmat_plane^dstmp2(m)*(Rmat_plane*AmatUD2_plane)^tmp2(m)*...
                     Rmat_plane^dstmp23*(Rmat_plane*AmatUD3_plane')^tmp3(m)*Rmat_plane^dstmp3(m)*(Rmat_plane*AmatUD3_plane)^tmp3(m)*real(Smat_plane); % second PGSE
...
if(nargout>1)
    ...
    if nargin < 3 
        ...
        Epert = GammaFiniteCylinders_MM_tPGSE(xpert,protocol);
        ...
        for i = 1:length(x)
            if x_deriv(i) ~= 0                  
                ...
                xpert(i) = xpert(i)*(1+dx);    
                Epert = GammaFiniteCylinders_MM_tPGSE(xpert,protocol,x_deriv);
                dEtdx = (Epert - E)/(xpert(i)*dx);
                J(:,i) = dEtdx;
            end
        end
