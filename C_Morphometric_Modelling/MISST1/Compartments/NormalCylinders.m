function [E,J]=NormalCylinders(x, protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating the NormalCylinders compartment.
% 
% [E,J]=NormalCylinders(x, protocol,x_deriv)
% 
...
if strcmp(approx,'GPD')
     ...
     if(nargout>1)
        ...
        if nargin < 3 
            ...
            for i = 1:length(x); % compute the derivatives for all model parameters
                ...
                Epert = NormalCylinders(xpert, protocol);
   ...
   fun_name = ['NormalCylinders_' approx '_' protocol.pulseseq]; 
   fun = str2func(fun_name);   
   if nargout == 1
        E = fun(x,protocol);
   else
