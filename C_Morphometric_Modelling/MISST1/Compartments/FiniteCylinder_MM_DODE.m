function [E J]=FiniteCylinder_MM_DODE(x,protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating a finite cylinder compartment.
% 
% [E,J]=FiniteCylinder_MM_DODE(x, protocol,x_deriv)
% 
    ...
    elseif protocol.phase == pi/2 || protocol.phase == -pi/2; 
        ...
        for m=1:M
               ...
               AmatUD2_plane=AmatU2_plane^G_ind2(m);
                 ...
                 mat21_plane = (Rmat_plane*AmatUD2_plane)^tmp21(m);   
                 ...
                 Ecal_plane=Ecal_plane*(mat11_plane*mat11_inv_plane^2*mat11_plane)^floor(Nosc(m))*... 
                     ...
                     mat21_inv_plane*mat21_plane*real(Smat_cyl); % second ODE
                     end
                 ePerp(m) = Ecal_cyl;
                 ePar(m) = Ecal_plane;
    ...
    tmp2 = tmp2 - 2*tmp2_rt;
       ...
       if length(G2up) >= 1  
           ...
           Ecal_cyl=Ecal_cyl*( mat1up_pos_cyl*(Rmat_cyl*AmatUD1_cyl)^tmp1(m)*mat1down_pos_cyl*...
             mat1up_neg_cyl*(Rmat_cyl*AmatUD1_cyl')^tmp1(m)*mat1down_neg_cyl)^Nosc(m)*... % first ODE
             Rmat_cyl^dstmp(m)*(mat2up_neg_cyl*(Rmat_cyl*AmatUD2_cyl')^tmp2(m)*mat2down_neg_cyl*...
             mat2up_pos_cyl*(Rmat_cyl*AmatUD2_cyl)^tmp2(m)*mat2down_pos_cyl)^Nosc(m)*real(Smat_cyl); % second ODE
       ...
       if length(G2up) >= 1  
           ...
           for it = 2:length(G1up)       
            ...
            mat2down_neg_plane = eye(size(Rmat_plane));
             ...
             if mod(Nhp(m),2) == 0 % even number of half periods
            Ecal_cyl=Ecal_cyl*mat1up_pos_cyl*(Rmat_cyl*AmatUD1_cyl)^tmp11(m)*mat1down_pos_cyl*...
                (mat1up_neg_cyl*(Rmat_cyl*AmatUD1_cyl')^tmp12(m)*mat1down_neg_cyl*...
                mat1up_pos_cyl*(Rmat_cyl*AmatUD1_cyl)^tmp12(m)*mat1down_pos_cyl)^(Nosc(m)-1)*...
                mat1up_neg_cyl*(Rmat_cyl*AmatUD1_cyl')^tmp12(m)*mat1down_neg_cyl*...
                mat1up_pos_cyl*(Rmat_cyl*AmatUD1_cyl)^tmp11(m)*mat1down_pos_cyl*...  % first ODE
                Rmat_cyl^dstmp(m)*mat2up_neg_cyl*(Rmat_cyl*AmatUD2_cyl')^tmp21(m)*mat2down_neg_cyl*...
                (mat2up_pos_cyl*(Rmat_cyl*AmatUD2_cyl)^tmp22(m)*mat2down_pos_cyl*...
                mat2up_neg_cyl*(Rmat_cyl*AmatUD2_cyl')^tmp22(m)*mat2down_neg_cyl)^(Nosc(m)-1)...
                *mat2up_pos_cyl*(Rmat_cyl*AmatUD2_cyl)^tmp22(m)*mat2down_pos_cyl*...
                mat2up_neg_cyl*(Rmat_cyl*AmatUD2_cyl')^tmp21(m)*mat2down_neg_cyl*real(Smat_cyl); % second ODE
            ...
            Ecal_cyl=Ecal_cyl*mat1up_pos_cyl*(Rmat_cyl*AmatUD1_cyl)^tmp11(m)*mat1down_pos_cyl*...
                (mat1up_neg_cyl*(Rmat_cyl*AmatUD1_cyl')^tmp12(m)*mat1down_neg_cyl*...
                mat1up_pos_cyl*(Rmat_cyl*AmatUD1_cyl)^tmp12(m)*mat1down_pos_cyl)^floor(Nosc(m))*...
                mat1up_neg_cyl*(Rmat_cyl*AmatUD1_cyl')^tmp11(m)*mat1down_neg_cyl*... % first ODE
                Rmat_cyl^dstmp(m)*mat2up_neg_cyl*(Rmat_cyl*AmatUD2_cyl')^tmp21(m)*mat2down_neg_cyl*...
                (mat2up_pos_cyl*(Rmat_cyl*AmatUD2_cyl)^tmp22(m)*mat2down_pos_cyl*...
                mat2up_neg_cyl*(Rmat_cyl*AmatUD2_cyl')^tmp22(m)*mat2down_neg_cyl)^floor(Nosc(m))...
                *mat2up_pos_cyl*(Rmat_cyl*AmatUD2_cyl)^tmp21(m)*mat2down_pos_cyl*real(Smat_cyl); % second ODE
...
if(nargout>1)
    ...
    if nargin < 3 
        ...
        Epert = FiniteCylinder_MM_DODE(xpert,protocol);
        ...
        for i = 1:length(x)
            if x_deriv(i) ~= 0                  
                ...
                xpert(i) = xpert(i)*(1+dx);    
                Epert = FiniteCylinder_MM_DODE(xpert,protocol,x_deriv);
                dEtdx = (Epert - E)/(xpert(i)*dx);
                J(:,i) = dEtdx;
            end
        end
