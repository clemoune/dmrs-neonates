function [E,J]=GammaFiniteAstroCylinders(x, protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating the GammaFiniteAstroCylinders compartment.
% 
% [E,J]=GammaFiniteAstroCylinders(x, protocol,x_deriv)
% 
% Description: Returns the diffusion measurement and the Jacobian with 
% respect to model parameters for a substrate consisting of isotropically 
% oriented finite cylinders with a Gamma distribution of radii and a diffusion 
% protocol specified in the input
% Substrate: Isotropically distributed impermeable finite cylinders with a Gamma
...
% x - size 4 vector of model parameters in SI units for GammaFiniteAstroCylinders:
    ...
    if(nargout>1)
        ...
        if nargin < 3          
            for i = 1:length(x); % compute the derivatives for all model parameters
                ...
                Epert = GammaFiniteAstroCylinders(xpert, protocol);
...
else % fo squences calculated MM method
