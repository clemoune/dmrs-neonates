function [E,J]=GammaAstroCylinders(x, protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating the GammaAstroCylinders compartment.
% 
% [E,J]=GammaAstroCylinders(x, protocol,x_deriv)
% 
% Description: Returns the diffusion measurement and the Jacobian with 
% respect to model parameters for a substrate consisting of isotropically 
% oriented cylinders with a Gamma distribution of radii and a diffusion 
% protocol specified in the input
% Substrate: Isotropically distributed impermeable cylinders with a Gamma
...
% x - size 3 vector of model parameters in SI units for GammaAstroCylinders:
...
if strcmp(approx,'GPD')
    ...
    if strcmp(protocol.pulseseq,'PGSE') || strcmp(protocol.pulseseq,'SWOGSE') || strcmp(protocol.pulseseq,'TWOGSE') || ...
       ...
       fun = str2func('AstroCylinders'); % integrate over orientation and sum over radii
   ...
   else % sequences that have variable orientation
    ...
    if(nargout>1)
        ...
        if nargin < 3          
            for i = 1:length(x); % compute the derivatives for all model parameters
                ...
                Epert = GammaAstroCylinders(xpert, protocol);
...
else % for sequences calculated with MM method
