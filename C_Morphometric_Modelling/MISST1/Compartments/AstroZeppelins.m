function [E,J]=AstroZeppelins(x, protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating the AstroZeppelins compartment.
% 
% [E,J]=AstroZeppelins(x, protocol,x_deriv)
% 
...
%       x(1) - diffusivity parallel to the zeppelin axis
%       x(2) - diffusivity perpendicular to the zeppelin axis
    ...
    if(nargout>1)
        ...
        if nargin < 3          
            for i = 1; % compute the derivatives for all model parameters
                ...
                Epert = AstroZeppelins(xpert, protocol);
    ...
    if nargin < 3          
        ...
        J(:,1) = dEtdD;        
            ...
            if x_deriv(1) ~= 0                 
                ...
                protocol.diff=1;   
                xpert(1) = xpert(1)*(1+dx);    
                Epert = AstroZeppelins(xpert,protocol,x_deriv);
