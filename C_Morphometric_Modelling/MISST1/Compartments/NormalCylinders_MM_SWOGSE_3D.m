function [E J]=NormalCylinders_MM_SWOGSE_3D(x,protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating a NormalCylinders compartment.
% 
% [E,J]=NormalCylinders_MM_SWOGSE_3D(x, protocol,x_deriv)
% 
...
if protocol.angle == 1 % gradient is only along x
    ...
    for indj = 1:length(protocol.A_cyl) 
        ...
        elseif protocol.diff==3
            Amat=protocol.Apertvar_cyl{indj};
            Smat=protocol.Spertvar_cyl{indj};
            Rmat=protocol.Rpertvar_cyl{indj};  
...
if(nargout>1)
    ...
    if nargin < 3 
        ...
        Epert = NormalCylinders_MM_SWOGSE_3D(xpert,protocol);
        ...
        for i = 1:length(x)    
            if x_deriv(i) ~= 0  
                ...
                xpert(i) = xpert(i)*(1+dx);    
                Epert = NormalCylinders_MM_SWOGSE_3D(xpert,protocol,x_deriv);
                dEtdx = (Epert - E)/(xpert(i)*dx);
                J(:,i) = dEtdx;
            end
        end
