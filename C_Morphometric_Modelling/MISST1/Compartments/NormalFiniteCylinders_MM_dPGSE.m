function [E J]=NormalFiniteCylinders_MM_dPGSE(x,protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating the NormalFiniteCylinders compartment.
% 
% [E,J]=NormalFiniteCylinders_MM_dPGSE(x, protocol,x_deriv)
% 
...
fib_dir = GetFibreOrientation('NormalFiniteCylinders',x);
...
if(nargout>1)
    ...
    if nargin < 3 
        ...
        Epert = NormalFiniteCylinders_MM_dPGSE(xpert,protocol);
        ...
        for i = 1:length(x)
            if x_deriv(i) ~= 0                  
                ...
                xpert(i) = xpert(i)*(1+dx);    
                Epert = NormalFiniteCylinders_MM_dPGSE(xpert,protocol,x_deriv);
                dEtdx = (Epert - E)/(xpert(i)*dx);
                J(:,i) = dEtdx;
            end
        end
