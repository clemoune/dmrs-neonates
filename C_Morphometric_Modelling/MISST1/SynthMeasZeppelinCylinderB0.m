function [E, J] = SynthMeasZeppelinCylinderB0(params,protocol,params_deriv)
%
% camino.m--------------------------------------------------------------
...
if nargout == 1           
        E = b0*((1-f)*Zeppelin(params(index1),protocol)+f*Cylinder(params(index2),protocol));
else 
    if nargin <3
    ...
    it = 0;
    if params_deriv(1) it = it+1; J(:,it) = b0*(E2 - E1); end % volume fraction
    if params_deriv(2) it = it+1; J(:,it) = b0*((1-f)*J1(:,1) + f*J2(:,1)); end % Dpar
    if params_deriv(3) it = it+1; J(:,it) = b0*((1-f)*J1(:,2)); end % Dperp
    if params_deriv(4) it = it+1; J(:,it) = b0*(f*J2(:,2)); end % radius
    if params_deriv(5) it = it+1; J(:,it) = b0*((1-f)*J1(:,3) + f*J2(:,3)); end % theta
    if params_deriv(6) it = it+1; J(:,it) = b0*((1-f)*J1(:,4) + f*J2(:,4)); end % phi 
    if params_deriv(7) it = it+1; J(:,it) = E; end % b0 
