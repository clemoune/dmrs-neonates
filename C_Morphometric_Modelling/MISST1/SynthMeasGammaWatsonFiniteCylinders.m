function [E J] = SynthMeasGammaWatsonFiniteCylinders(params,protocol,params_deriv)
%
% camino.m--------------------------------------------------------------
% Normalized diffusion signal simulating a GammaWatsonFiniteCylinders model.
% 
% [E,J]=SynthMeasGammaWatsonFiniteCylinders(params, protocol,params_deriv)
% 
% Description: Returns the diffusion measurement and the Jacobian with 
% respect to model parameters for a one compartment model cosisting of
% finite cylinders with Gamma distributed radii and a Watson distribution of
...
%       params(4) - eccentricity (ratio between cylinder length and diameter -
% is the same for all sizes)
%       params(5) - kappa: cncentration parameter of Watson distribution
...
%     
    if nargout == 1           
        E = GammaWatsonFiniteCylinders(params,protocol);
    else
        if nargin == 3
        [E Jinit] = GammaWatsonFiniteCylinders(params,protocol,params_deriv);
        ...
        [E J] = GammaWatsonFiniteCylinders(params,protocol);    
        end
    end
end