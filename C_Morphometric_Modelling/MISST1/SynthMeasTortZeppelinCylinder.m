function [E, J] = SynthMeasTortZeppelinCylinder(params,protocol,params_deriv)
%
% camino.m--------------------------------------------------------------
...
if nargout == 1           
        E = (1-f)*Zeppelin(params_zep,protocol)+f*Cylinder(params_cyl,protocol);
else 
    if nargin <3
    ...
    if params_deriv(3) it = it+1; J(:,it) = f*J2(:,2); end % radius
    if params_deriv(4) it = it+1; J(:,it) = (1-f)*J1(:,3) + f*J2(:,3); end % theta
    if params_deriv(5) it = it+1; J(:,it) = (1-f)*J1(:,4) + f*J2(:,4); end % phi 
