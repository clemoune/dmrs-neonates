function [E, J] = SynthMeasTortZeppelinCylinderDotBall(params,protocol,params_deriv)
%
% camino.m--------------------------------------------------------------
% Normalized diffusion signal simulating a TortZeppelinCylinderDotBall model.
% 
...
% CSF - isotropic free diffusion
%
% Parameters:   
% E - diffusion signal
% J - Jacobian of the diffusion signal with respect to model parameters
% params - size 8 vector of model parameters in SI units for Cylinder:
...
%       params(5) - volume fraction of isotropic compartment
...
if nargout == 1           
        E = (1-ficvf)*(1-irfrac)*(1-fiso)*Zeppelin(params_zep,protocol)+ficvf*(1-irfrac)*(1-fiso)*Cylinder(params_cyl,protocol)+ ...
    ...
    E = (1-ficvf)*(1-irfrac)*(1-fiso)*E1+ficvf*(1-irfrac)*(1-fiso)*E2+ ...
    ...
    if params_deriv(1) % ficvf volume fraction, including turtuosity term
        it = it+1; 
        J(:,it) = (1-irfrac)*(1-fiso)*E2 - (1-irfrac)*(1-fiso)*E1 + (1-irfrac)*(1-fiso)*(1-ficvf)*J1(:,2)*(-params_zep(1)); 
    end 
    if params_deriv(2) % di, including turtuosity term
        it = it+1; 
        J(:,it) = (1-irfrac)*(1-ficvf)*(1-fiso)*J1(:,1) + ficvf*(1-irfrac)*(1-fiso)*J2(:,1) + (1-irfrac)*(1-ficvf).^2*(1-fiso)*J1(:,2) ; 
    end 
    if params_deriv(3) % radius
        it = it+1;
        J(:,it) = ficvf*(1-irfrac)*(1-fiso)*J2(:,2); 
    end 
    if params_deriv(4) % irfrac
        it = it+1;
        J(:,it) =-(1-ficvf)*(1-fiso)*E1 - ficvf*(1-fiso)*E2 + (1-fiso)*E3;
    end
    if params_deriv(5) % fiso
        it = it+1; 
        J(:,it) = -(1-ficvf)*(1-ficvf)*E1 - ficvf*(1-irfrac)*E2 -irfrac*E3 +E4; 
    end 
    ...
    if params_deriv(7) % theta
        it = it+1; 
        J(:,it) = (1-irfrac)*(1-ficvf)*(1-fiso)*J1(:,3) + (1-irfrac)*ficvf*(1-fiso)*J2(:,3); 
    end 
    if params_deriv(8) % phi
        it = it+1; 
        J(:,it) = (1-irfrac)*(1-ficvf)*(1-fiso)*J1(:,4) + (1-irfrac)*ficvf*(1-fiso)*J2(:,4); 
    end  
