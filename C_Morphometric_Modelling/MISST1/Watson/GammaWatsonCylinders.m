function [E,J]=GammaWatsonCylinders(x, protocol,x_deriv)
%
% camino.m--------------------------------------------------------------
% Diffusion signal simulating a GammaWatsonCylinders compartment.
%
% [E,J]=GammaWatsonCylinders(x, protocol,x_deriv)
% 
% Description: Returns the diffusion measurement and the Jacobian with 
% respect to model parameters for a substrate of cylinders with a Gamma
% distribution of sizes and a Watson distribution of orientations and a 
% diffusion protocol specified in the input 
%
% Substrate: Cylinders with Gamma distributed radii and Watson orientation
...
%       x(3) - shape parameter of Gamma distribution
...
if strcmp(approx,'GPD')
    ...
    if  strcmp(protocol.pulseseq,'PGSE') || strcmp(protocol.pulseseq,'SWOGSE') || strcmp(protocol.pulseseq,'TWOGSE') || ...
        ...
        fun = str2func('WatsonCylinders');
        

        E0 = zeros(length(protocol.smalldel),length(R));
        for i = 1:length(R)  
            params = [x(1) R(i) x(4:end)];
        ...
        maindir = GetFibreOrientation('GammaWatsonCylinders', x);
    ...
    if(nargout>1)
        ...
        if nargin < 3          
            for i = 1:length(x); % compute the derivatives for all model parameters
                ...
                Epert = GammaWatsonCylinders(xpert, protocol);
    ...
    fun = str2func(['GammaCylinders_' approx '_' protocol.pulseseq]);
    if isfield(protocol,'Ndir')
        Ndir = protocol.Ndir;
    else
        Ndir = 100;  
    ...
    if nargout == 1
        ...
        theta_idx = GetParameterIndex('GammaWatsonCylinders','theta');
        xpertth = x; xpertth(theta_idx) = x(theta_idx)+dx;    
        maindir_pertth = GetFibreOrientation('GammaWatsonCylinders', xpertth); 
        ...
        phi_idx = GetParameterIndex('GammaWatsonCylinders','phi');
        xpertphi = x; xpertphi(phi_idx) = x(phi_idx)+dx;    
        maindir_pertphi = GetFibreOrientation('GammaWatsonCylinders', xpertphi); 
